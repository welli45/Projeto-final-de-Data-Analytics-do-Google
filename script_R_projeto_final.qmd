...
### Tabela 1489: População residente, por cor ou raça, segundo o sexo a a religião - Resultados Gerais da Amostra

```{r}
#| message: false
dados <- get_sidra(x = 1489,
                   period = "2010",
                   geo = "State",
                   header = T,
                   format = 3)
...
```
Verificando quais informações temos sobre a variável unidade_da_federação
```{r}
unique(dados$unidade_da_federacao)
```
...
```{r}
dados <- dados %>% 
  filter(cor_ou_raca != "Total", sexo != "Total", religiao != "Total")
```
